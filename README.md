...
This project is a high-performance microservice designed for efficient template management, leveraging **TypeScript**, **Node.js** with **Fastify**, and **Prisma ORM** for robust data interactions with **PostgreSQL**. It features automatic versioning of templates and comprehensive API endpoints for CRUD operations. This service is ideal for systems requiring dynamic and version-controlled content, such as email or notification templates.

## Features

- ⚡️ **High-Performance API**: Built with Fastify for blazing-fast request handling and low overhead.
- 🔒 **Type-Safe Database Operations**: Utilizes Prisma ORM for a modern, type-safe, and intuitive way to interact with PostgreSQL.
- 🔄 **Automatic Template Versioning**: Every update to a template's `subject`, `body`, or `version_number` automatically creates a new version entry, ensuring historical data is preserved.
- 🔍 **Advanced Querying**: Supports pagination, language filtering, and keyword search on template `name` and `subject`.
- ✨ **Clean Architecture**: Organized codebase with clear separation of concerns (controllers, routes, plugins).
- ✅ **Robust Error Handling**: Consistent error responses with appropriate HTTP status codes and detailed messages.
